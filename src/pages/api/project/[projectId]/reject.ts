...
export const POST: APIRoute = async ({params, locals}) => {
    ...
    // User can only reject project belonging to him
    if (project.customerId !== user.id) {
        return new Response(null, {status: 403});
    }

    if (project.state !== projectState.COMPLETED) {
        return new Response("Only COMPLETED projects can be rejected.", {status: 400});
    }

    const updated = await changeProjectState(project.id, projectState.ASSIGNED);
